...
class IE(nn.Module):
    def __init__(self, dataloader, additivity_order=None, narray_op='Algebraic', preprocessing='PreprocessingLayerNone', device='cpu'):
        ...
        if self.preprocessing not in ['PreprocessingLayerPercentile', 'PreprocessingLayerStandardDeviation', 'PreprocessingLayerMaxMin', 'Random', 'PreprocessingLayerNone']:
            raise ValueError('Valid inputs are as follows / PreprocessingLayerPercentile, PreprocessingLayerStandardDeviation, PreprocessingLayerMaxMin, Random', 'PreprocessingLayerNone')
    ...
    def fit_and_valid(self, train_loader, test_loader, criterion, optimizer, epochs=100, regularization=True, mono_lambda=0.2):
        ...
        for epoch in range(epochs):
            ...
            for i, (images, labels, sample_weights) in enumerate(train_loader):
                ...
                # Compute the loss
                loss = torch.dot(sample_weights.reshape(-1), ((outputs - labels).reshape(-1))**2) / labels.shape[0] #Loss without regularization term (for output)
            ...
            with torch.no_grad():
                for images, labels, sample_weights in test_loader:
                    ...
                    outputs = self(images)
                    loss = torch.dot(sample_weights.reshape(-1), ((outputs - labels).reshape(-1))**2) / labels.shape[0]
    ...
    def interaction_indices(self):
        ...
        for i in range(0, len(data)):
            ...
            for j in range(0, len(data)):
                ...
                B = all_sets[j]
                if  set(B) >= set(A):
                    f += data[j] / (len(B) - len(A) + 1)
            interaction_.append(f)
        dict_fuzzy = dict(zip(all_sets, interaction_))
